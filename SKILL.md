...
description: "Have the main thread perform the GPT-6 Astra lead-orchestrator role, delegate difficult but clearly bounded work to GPT-6 Sol Max, and assign clear, repeatable work to GPT-6 Luna Max. Use when the user requests Astra orchestration, Sol Max or Luna Max subagents, tiered multi-agent coding, parallel code review, module analysis, independent feature implementation, testing, debugging, or result integration. Do not infer or require switching the main model; before delegation, check only the required subagent model and reasoning combinations."
---

# Three-Tier Agent Symphony

Have the current main thread perform Astra's orchestration role: understand the objective, break down tasks, make architectural decisions, verify completion, and integrate results. Delegate difficult but clearly bounded work to Sol Max, and clear, repeatable, easily verifiable work to Luna Max.
...
3. Proceed to Section 2 only after confirming that Sol Max and Luna Max are available.

Every time Sol Max or Luna Max is launched, pass the corresponding `model` and `reasoning_effort` values directly to the subagent tool. Set `fork_turns = "none"` for Luna Max and by default for Sol Max. For Sol Max, use `fork_turns = "2"` only when its task directly depends on the latest conversation and restating that context would reduce clarity. Never omit `fork_turns`, and never use `fork_turns = "all"`.
...
If `Sol Max`, `Luna Max`, or either required model/reasoning combination is unavailable, immediately stop the entire workflow, not just delegation. Apart from checking subagent capabilities and explaining how to enable them, do not run tools or advance the substantive task. This includes:
...
- Sol Max requires `gpt-6-sol` with `max`.
...
| Sol Max | Difficult but self-contained module analysis, nontrivial independent implementation, in-depth code review, security or concurrency reasoning, and complex root-cause investigation |
...
- Sol Max should use `"none"`; it may use `"2"` only for a task that directly depends on the latest conversation.
...
Use a hub-and-spoke structure by default: Astra delegates directly to Sol Max and Luna Max, and both report directly to Astra. Do not let Sol Max manage Luna Max unless Sol Max has been authorized to take full ownership of an independent subsystem.
...
- Sol Max implements a complex feature, Luna Max creates or runs focused tests, and Astra performs the final integration.
- Luna Max completes a clearly specified change, Sol Max performs an in-depth review, and Astra decides whether to accept it.
- Luna Max gathers reproduction and log evidence, Sol Max analyzes the root cause, and Astra decides on the cross-module fix.
...
5. Reports the completed work, validation results, remaining risks, and the actual division of work among Astra, Sol Max, and Luna Max in the final response.
