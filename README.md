...
A Codex skill that has the current main thread perform the Astra lead-orchestrator role while delegating independent subtasks to Sol Max and Luna Max based on the nature of the work.
...
- Verify that Sol Max and Luna Max are available before starting substantive work.
...
- Launch Sol Max and Luna Max by passing the exact model and reasoning values directly to the subagent tool.
- Choose the subagent model and reasoning effort first, then derive the task-name suffix from those exact values: `_sol_max` or `_luna_max`.
- Use `fork_turns: "none"` for Luna Max and by default for Sol Max. Sol Max may use `"2"` only when its task directly depends on the latest conversation; never omit the field or use `"all"`.
...
- Subagents can use `gpt-6-sol` with `max` reasoning and `gpt-6-luna` with `max` reasoning.
...
```text
Use the Three-Tier Agent Symphony to review this project: Astra handles integration, Sol Max performs the architecture and security review, and Luna Max runs tests and organizes the errors.
```
